...
class Credis_Sentinel
{
     ...
     */
    public function createCluster($name, $password = '', $db = 0, $replicas = 128, $selectRandomSlave = true, $writeOnly = false)
    {
        $clients = array();
        ...
        foreach ($slaves as $slave) {
            if (!strstr($slave[9], 's_down') && !strstr($slave[9], 'disconnected')) {
                $workingClients[] = array('host' => $slave[3], 'port' => $slave[5], 'master' => false, 'db' => $db, 'password' => $password);
            }
        }
        if (count($workingClients) > 0) {
            if ($selectRandomSlave) {
                if (!$writeOnly) {
                    $workingClients[] = array('host' => $master[3], 'port' => $master[5], 'master' => false, 'db' => $db, 'password' => $password);
                }
        ...
        $clients[] = array('host' => $master[3], 'port' => $master[5], 'db' => $db, 'master' => true, 'write_only' => $writeOnly, 'password' => $password);
     ...
     * @return Credis_Cluster
     */
    public function getCluster($name, $password = '', $db = 0, $replicas = 128, $selectRandomSlave = true, $writeOnly = false)
    {
        if (!isset($this->_cluster[$name])) {
            $this->_cluster[$name] = $this->createCluster($name, $password, $db, $replicas, $selectRandomSlave, $writeOnly);
        }
