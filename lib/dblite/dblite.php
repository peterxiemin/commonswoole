...
class DBLite
{
	...
	public function __construct($config, $id = null)
	{
		...
		{
			throw new \lib\dblite\Exception('PDO extension is not loaded.');
		}
		if (!isset($config['DBLite']['adapter']))
		{
			throw new \lib\dblite\Exception('config error:lack adapter');
		}
		$this->_config = $config;
		$adapter = $this->_config['DBLite']['adapter'];
		if (!isset($this->_config[$adapter]))
		{
			throw new \lib\dblite\Exception('config error:lack adapter config');
		}
		if (empty($id))
		{
			$id = isset($this->_config['DBLite']['defaultConnection']) ? $this->_config['DBLite']['defaultConnection'] : '';
		}
		$adapterClass = '\\lib\\dblite\\adapter\\' . $adapter;
		$this->_adapter = new $adapterClass($this->_config[$adapter], $id);
	...
	public function query($sql, array $bind = array())
	{
		...
		if(!$succ)
		{
			...
			throw new \lib\dblite\Exception("Failed to execute query statement: :sql\nError Code: :errcode\nError Info: :errinfo\n", array(':sql' => $sql, ':errcode' => $sth->errorCode(), ':errinfo' => $errorInfo[2]));
		}
	...
	public function execute($sql, array $bind = array())
	{
		...
		{
			$errorInfo = $sth->errorInfo();
			throw new \lib\dblite\Exception("Failed to execute SQL: :sql\nError Code: :errcode\nError Info: :errinfo\n", array(':sql' => $sql, ':errcode' => $sth->errorCode(), ':errinfo' => $errorInfo[2]));
		}
		$this->_lastStatement = $sth;
		return $sth->rowCount();
	}
