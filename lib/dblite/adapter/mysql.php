 ...
 */
namespace lib\dblite\adapter;

class Mysql extends \lib\dblite\Adapter
{
	private $_pdo;
	...
	public function __construct($config, $id)
	{
		...
		{
			throw new \lib\dblite\Exception('config file error,please specify database id');
		}
		$this->_config = $config;
		...
		foreach($required as $key)
		{
			foreach ($this->_connections as $connection)
			{
				...
				{
					throw new \lib\dblite\Exception('MySQL adapter needs the following information: host, username, password and dbname, please check your configuration.');
				}
			}
		}
		if(!extension_loaded('pdo_mysql'))
		{
			throw new \lib\dblite\Exception('pdo_mysql extension is not loaded.');
		}
	}
	...
	public function createPDO()
	{
		...
		if(!$pdo)
		{
			throw new \lib\dblite\Exception('Connection failed: ' . $this->_lastErrorInfo);
		}
		return $pdo;
	}
	...
	public function explain($sql)
	{
		...
		{
			throw new \lib\dblite\Exception('Mysql cannot explain non-SELECT SQL statements.');
		}
