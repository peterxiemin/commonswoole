...
class SSWrapper
{
     ...
     * @throws Exception
     */
    private function _disPatch($c_arr)
    {
        if ($c_arr && isset($c_arr['path_info']) && isset($c_arr['cname']))
        {
            $c = '\\logic'.str_replace('/', '\\', $c_arr['path_info']).''.$c_arr['cname'];
            return new $c;
        }
        else {
            throw new \Exception('path_info or cname is invaild');
        }
